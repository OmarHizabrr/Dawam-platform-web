...
export default function Statistics(props) {
  ...
  useEffect(() => {
    FirebaseServices.getGeneralStatistics()
      .then((data) => {
        setData(data);
        var q = [];
        var l = [];

        data?.qulaifications?.map((item) => {
        ...
        setDlabels(dl);
        setDiscData([
          (data.idealTime?.[0]?.ideal_time || 0) -
            (data.workHours?.[0]?.workHours || 0),
          parseFloat(data.workHours?.[0]?.workHours || 0),
        ]);
      })
    ...
    <Layout className="stat-layout">
      ...
      >
        <Col
          ...
          span={6}
          style={{ padding: "20px", color: "#fff", fontSize: "16px" }}
        >
          <div
            style={{
              padding: "10px 20px",
          ...
          >
            ...
            <div style={{ fontSize: "12px", marginTop: "10px" }}>
              آخر توظيف منذ{" "}
              {data.latest_assignment
                ? data.latest_assignment[0].assignment_date
                : ""}
            </div>
          </div>
        </Col>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: "12px", marginTop: "10px" }}>
              متوسط الموظفين لكل إدارة {data.dept_emp_avg}
            </div>
          </div>
        </Col>
        <Col
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: "22px" }}>
              {Math.round(data.age_avg)} عاماً
            </div>
            <div style={{ fontSize: "12px", marginTop: "10px" }}>
              {data.youngest} أصغر الموظفين عمراً
            </div>
          </div>
        </Col>
        <Col
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: "22px" }}>
              {data.attendance_count} موظفاً
            </div>
            <div style={{ fontSize: "12px", marginTop: "10px" }}>
              نسبة حضور الموظفين اليوم {data.attendance_percent}%
            </div>
          </div>
        </Col>
      </Row>
