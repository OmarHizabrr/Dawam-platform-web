...
export default function TasksAccounts(props) {
  ...
  /// دالة حذف السنوية
  const handleDeleteTask = (fieldName, remove) => {
    // عرض رسالة تأكيد محسّنة
    if (
      window.confirm("❗ هل أنت متأكد أنك تريد حذف هذا السطر؟ لا يمكن التراجع!")
    ) {
      remove(fieldName);
      message.success("✅ تم حذف السطر بنجاح!", 2); // رسالة نجاح تختفي بعد ثانيتين
    }
  };
  ...
  const showAccount = (user_id) => {
    ...
    axios
      .get(
        `${Env.HOST_SERVER_NAME}get-tasks-statment/${user_id}/${currentYear}`
      )
  ...
  useEffect(() => {
    ...
    var records;
    axios
      .get(Env.HOST_SERVER_NAME + "get-emp-names")
      .then((response) => {
    ...
    axios
      .get(Env.HOST_SERVER_NAME + "get-tasks-types-re")
      .then((response) => {
    ...
    if (end != "")
      axios
        .get(Env.HOST_SERVER_NAME + "get-rest-tasks/" + currentYear)
        .then((response) => {
          if (
            ...
            emp.map((user, index) => {
              ...
              var vacDetails = "";
              tasks.map((task) => {
                var dur = response.data.tasks?.filter(
                  (record) =>
                    record.uid == user.value && record.vid == task.value
                );
                ...
                if (dur.length > 0)
                  dur =
                    dur[0].rest < 0
                      ? "-"
                      : "" +
                        Math.abs(parseInt(dur[0].rest / 60)) +
                        ":" +
                        Math.abs(dur[0].rest % 60);
                else dur = 0;
            ...
            var json = JSON.parse(
              vacData.substring(0, vacData.length - 1) + "]"
            );
...
*/
    ...
    axios
      .get(Env.HOST_SERVER_NAME + "get-annualy-tasks-report/2/" + currentYear)
      .then((response) => {
    ...
    if (filters) {
      Object.keys(filters).forEach((key) => {
        if (filters[key] != null) {
          ...
          setPAnnTasks(
            annTasks.filter((item) => filters[key].includes(item[key]))
          );
        } else setPData(data);
  ...
  const exportToExcel = (type, fn, dl) => {
    ...
    if (elt) {
      ...
      return dl
        ...
        : excel.writeFile(wb, fn || "كشف أرصدة الإجازات." + (type || "xlsx"), {
            bookSST: true,
            type: "base64",
            cellStyles: true,
          });
  ...
  const showUsersDebt = () => {
    setLoadUsers(true);
    axios
      .get(Env.HOST_SERVER_NAME + "get-users-long-debts/")
      .then((response) => {
        setLoadUsers(false);

        var tasks = response.data;
        const finalData = tasks.map((item) => ({
          ...item,
          amount: gform.getFieldValue("amount"),
          task_id: gform.getFieldValue("task_id"),
          type: gform.getFieldValue("type"),
        }));
  ...
  const onFinish = () => {
    setSaving(true);
    axios
      .post(Env.HOST_SERVER_NAME + "add-balance-tasks", form.getFieldsValue())
      .then((response) => {
        setSaving(false);
        setIsModalVisible(false);
        form.resetFields();
  ...
  const onDFinish = () => {
    ...
    axios
      .post(
        Env.HOST_SERVER_NAME + "discount-task-account",
        dform.getFieldsValue()
      )
      .then((response) => {
        setDSaving(false);
        setIsDModalVisible(false);
  ...
  return (
    <Card>
      {/* Modal إضافة رصيد إجازة موظف */}
      <Modal
        centered
        confirmLoading={saving}
        title="إضافة رصيد إجازة موظف"
      ...
      >
        <Form form={gform} layout="vertical">
          <Row gutter={16}>
            ...
            <Col span={6}>
              <Form.Item name="type" label="نوع الرصيد">
                <Select
                  placeholder="اختر نوع الرصيد"
                  options={types.filter((e) => e.parent === 34)}
                />
              </Form.Item>
            </Col>
            <Col span={6}>
              <Form.Item name="amount" label="الرصيد بالدقائق">
                <InputNumber
                  style={{ width: "100%" }}
                  placeholder="الرصيد بالدقائق"
                />
              </Form.Item>
            </Col>
            <Col span={6}>
          ...
          <Row justify="end" style={{ marginBottom: "16px" }}>
            <Button loading={loadUsers} onClick={showUsersDebt} type="primary">
              جلب الموظفين
            </Button>
           </Row>
          <Divider />
                    ...
                    >
                      <Col span={4}>
                        <Form.Item
                          ...
                          label="اسم الموظف"
                          rules={[
                            { required: true, message: "يرجى اختيار الموظف" },
                          ]}
                        >
                          <Select
                      ...
                      <Col span={4}>
                        <Form.Item
                          ...
                          rules={[
                            {
                              required: true,
                              message: "يرجى اختيار نوع الرصيد",
                            },
                          ]}
                        >
                          <Select
                      ...
                      <Col span={6}>
                        ...
                        >
                          <Input.TextArea placeholder="ملاحظات" rows={1} />
                        </Form.Item>
                      </Col>

                      <Col span={2} style={{ textAlign: "center" ,marginTop: "25px" }}>
                        <Button
                          danger
                          type="text"
                          icon={<DeleteOutlined style={{ fontSize: "24px" }} />}
                          onClick={() => handleDeleteTask(name, remove)}
                          title="حذف السطر"
                        />
                      </Col >
      ...
      {/* Modal خصم رصيد إجازة */}
      <Modal
        centered
        confirmLoading={dsaving}
        title="خصم رصيد إجازة"
      ...
      <div style={{ marginBottom: "10px" }}>
        <Row justify="space-between" align="middle">
          ...
          <Col>
            ...
            />
              <Button
                onClick={() => setIsDModalVisible(true)}
                style={{
              ...
              >
                <FormOutlined /> إضافة رصيد
              </Button>
              <Button onClick={() => exportToExcel("xlsx")} type="primary">
                <ExportOutlined />
              </Button>
              <Button
                loading={loadReport}
                onClick={printAnnualyReport}
                type="primary"
                style={{
            ...
            <tbody>
                    ...
                    <>
                        ...
                        return (
                          ...
                          >
                            ...
                            {tasksTypes.map((task, index) => {
                              var taskAmount = item[task.label]?.replace(
                                /(\d{1,2}:\d{2}):\d{2}/,
                                "$1"
                              );
                              ...
                              var finalTask =
                                taskAmount == 0
                                  ? 0
                                  : parseInt(taskSplit[0] * 60) +
                                    parseInt(taskSplit[1]);

                              ttasksTypes[index] =
                                ttasksTypes[index] + parseInt(finalTask);
                              tttasksTypes[index] =
                                tttasksTypes[index] + parseInt(finalTask);
                      ...
                      >
                        ...
                        {tasksTypes.map((task, index) => {
                          return (
                            <td>
                              {parseInt(ttasksTypes[index] / 60) +
                                ":" +
                                (ttasksTypes[index] % 60)}
                            </td>
                          );
                        })}
                        <td>
              ...
              >
                ...
                {tasksTypes.map((task, index) => {
                  return (
                    <td>
                      {parseInt(tttasksTypes[index] / 60) +
                        ":" +
                        (tttasksTypes[index] % 60)}
                    </td>
                  );
                })}
                <td>
