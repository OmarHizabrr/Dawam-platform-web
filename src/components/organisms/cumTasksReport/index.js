...
export default function CumTasksReport(props) {
    ...
    if (data.length > 0 && empNames.length > 0 && tasksTypes.length > 0) {
      ...
      empNames.map((user, index) => {
        vacData +=
          ...
          user.value +
          '","category":"' +
          data?.filter((record) => record.uid == user.value)[0]?.category +
          '","job":"' +
          data?.filter((record) => record.uid == user.value)[0]?.job +
          '",';
      ...
      const ncolumns = [
        {
          ...
          onFilter: (value, record) => {
            ...
            return matchesFilterValue(record.empName, value);
          },
  ...
  useEffect(() => {
    ...
    FirebaseServices.getTasksTypesRe()
      .then((data) => {
        tasks = data;
        setTasksTypes(data);
      })
    ...
    if (end != "")
      FirebaseServices.getCumTasks(start, end)
        .then((dataBody) => {
          console.log(dataBody);
          if (
            dataBody.tasks?.length > 0 &&
            emp?.length > 0 &&
            tasks?.length > 0
          ) {
            var vacData = "[";
            emp.map((user, index) => {
              vacData +=
                ...
                user.value +
                '","category":"' +
                dataBody.tasks?.filter(
                  (record) => record.uid == user.value
                )[0]?.category +
                '","job":"' +
                dataBody.tasks?.filter(
                  (record) => record.uid == user.value
              ...
              var vacDetails = "";
              tasks.map((task) => {
                var dur = dataBody.tasks?.filter(
                  (record) =>
                    record.uid == user.value && record.vac_name == task.label
                );
                if (dur.length > 0) dur = dur[0].vac_duration;
                else dur = 0;
                ...
                <th colSpan={13}>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        كشف الإجازات لشهر {currentMonth}
                      </h1>
                      <h2
                        style={{
                          fontSize: " 14px",
