...
import { Badge, Button, Card, DatePicker, Layout, Modal, Table } from "antd";
...
export default function UsersPerformance(props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (last_occ, record) => {
        ...
        return new Date(max_att) > new Date(max_leave) ? (
          <>
            <Badge style={{ marginRight: "5px" }} status="success" />
            {" متواجد الآن"}
          </>
        ) : (
    ...
    },
    {
      ...
      ellipsis: false,
      render: (attendance_rate, record) =>
        Math.round(attendance_rate * 100) + "%",
    },
    {
      title: "انضباط الحضور",
      dataIndex: "att_rate",
      key: "att_rate",
      sorter: (a, b) =>
        Math.round(Math.round(a.att_rate * 100) * a.attendance_rate) -
        Math.round(Math.round(b.att_rate * 100) * b.attendance_rate),
      sortOrder: sortedInfo.columnKey === "att_rate" && sortedInfo.order,
      ellipsis: false,
      render: (att_rate, record) =>
        Math.round(Math.round(att_rate * 100) * record.attendance_rate) + "%",
    },
    {
      ...
      key: "leave_rate",
      sorter: (a, b) =>
        Math.round(Math.round(a.leave_rate * 100) * a.attendance_rate) -
        Math.round(Math.round(b.leave_rate * 100) * b.attendance_rate),
      sortOrder: sortedInfo.columnKey === "leave_rate" && sortedInfo.order,
      ellipsis: false,
      render: (leave_rate, record) =>
        Math.round(Math.round(leave_rate * 100) * record.attendance_rate) + "%",
    },
    {
      ...
      ellipsis: false,
      render: (lateTimes, record) =>
        parseInt(lateTimes / 60 / 60) + ":" + (parseInt(lateTimes / 60) % 60),
    },
    {
      ...
      ellipsis: false,
      render: (bonusTime, record) =>
        parseInt(bonusTime / 60 / 60) + ":" + (parseInt(bonusTime / 60) % 60),
    },
    {
      ...
      key: "total_rate",
      sorter: (a, b) =>
        totalRatePercent(a) - totalRatePercent(b),
      sortOrder: sortedInfo.columnKey === "total_rate" && sortedInfo.order,
      ellipsis: false,
      render: (col, record) => totalRatePercent(record) + "%",
    },
    {
      title: "سجل الحضور",
      key: "action",
  ...
  useEffect(() => {
    ...
    FirebaseServices.getUsersPerformanceRank(start, end)
      .then((data) => {
        if (location.pathname == "/profile/dept-performance") {
          var dt = data.filter(
            (record) => record.category == props.user.category.name
          );
          setData(dt);
          setPData(dt);
        } else {
  ...
  const handleChange = (pagination, filters, sorter) => {
    ...
    if (sorter?.order && typeof sorter.column?.sorter === "function") {
      workingData = [...workingData].sort((a, b) => sorter.column.sorter(a, b));
            ...
            >
              <thead>
                ...
                >
                  ...
                  <th style={{ fontWeight: "100" }}>الرقم الوظيفي</th>
                ...
                {pdata.map((item) => (
                  <tr
                    style={{
                      height: " 25px",
                      backgroundColor:
                        pdata.indexOf(item) % 2 != 0 ? "#e6e6e6" : "#fff",
                    }}
                  >
                    ...
                    <td>
                      {Math.round(
                        (Math.round(item.attendance_rate * 100) +
                          Math.round(
                            Math.round(item.att_rate * 100) *
                            item.attendance_rate
                          ) +
                          Math.round(
                            Math.round(item.leave_rate * 100) *
                            item.attendance_rate
                          )) /
                        3
                      ) + "%"}
