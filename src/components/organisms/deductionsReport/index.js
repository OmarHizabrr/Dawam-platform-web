...
export default function deductionsReport(props) {
  ...
  useEffect(() => {
    ...
    if (end != "")
      FirebaseServices.getDeductionsReport()
        .then((data) => {
          setDedTypes(data.types);
          deductions = data.types;
          if (
            ...
            emp.map((user, index) => {
              ...
              var vacDetails = "";
              deductions.map((task) => {
                var dur = data.deductions?.filter(
                  (record) =>
                    record.uid == user.value && record.ded_name == task.label
                );
                if (dur.length > 0) dur = dur[0].ded_amount;
                else dur = 0;
                ...
                <th colSpan={13}>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        خلاصة الاستقطاعات لشهر {currentMonth}
                      </h1>
                      <h2
                        style={{
                          fontSize: " 14px",
                        ...
                        return (
                          ...
                          >
                            ...
                            {dedTypes?.map((task, index) => {
                              ...
                              ttasksTypes[index] =
                                ttasksTypes[index] + parseInt(taskAmount);
                              tttasksTypes[index] =
                                tttasksTypes[index] + parseInt(taskAmount);
