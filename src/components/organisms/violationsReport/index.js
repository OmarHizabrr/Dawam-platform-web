...
export default function ViolationsReport (props){
  ...
  const getVacDuration=(user_id,vac_name)=>{
    for(var i = 0; i < data.length; i++)
      if(data[i].user_id == user_id && data[i].vio_name == vac_name ) return data[i].vio_count;  
      return 0;
  }
   ...
   return ncolumns;
    ...
    useEffect(() => {
      axios.get(Env.HOST_SERVER_NAME+'get-emp-names')
          ...
          setLoad(true);
    if(end!='')
    axios.get(Env.HOST_SERVER_NAME+'get-cum-violations/'+start+'/'+end)
    .then(response => {
...
return (
    ...
    <Table loading={load} columns={getColumnsVac()} scroll={{x: '1000px' }} dataSource={getOrganizedVacations()} onChange={function(){handleChange();}} />
