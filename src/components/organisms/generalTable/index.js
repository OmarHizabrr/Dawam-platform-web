...
import firstTrophyImage from "../../../assets/images/profileTrophies/first_today.png";
import secondTrophyImage from "../../../assets/images/profileTrophies/second.png";
import trophyImage from "../../../assets/images/profileTrophies/tag.png";
import thirdTrophyImage from "../../../assets/images/profileTrophies/third.png";
...
export default function generalTable(props) {
  ...
  // دالة لتحديد الصورة حسب الترتيب
  const getTrophyImage = (rank) => {
    switch (rank) {
      case 1:
        return firstTrophyImage;
      case 2:
        return secondTrophyImage;
      case 3:
        return thirdTrophyImage;
      default:
        return trophyImage;
    }
  };
  ...
  useEffect(() => {
    ...
    FirebaseServices.getAllUsersLog(today)
      .then((dataBody) => {
        const joinedData = dataBody["users"]?.map((user) => {
          const userAttendance = dataBody["logs_test"]?.find(
        ...
        const parseTimeString = (timeString) => {
          ...
          adjustedHours =
            meridiem === "PM" ? adjustedHours + 12 : adjustedHours;
        ...
        setAbData(abdata);
        const latedata = data.filter(
          (item) => item.attendance?.startLateTime < 0
        );
        setLateData(latedata);
        const timelydata = data.filter(
          (item) => item.attendance?.startLateTime >= 0
        );
        setTimelyData(timelydata);

        setAttAvg(
          dataBody["users"]?.length ? Math.round(
            (data.filter((item) => item.attendance != null).length /
              dataBody["users"].length) *
              100
          ) : 0
        );
  ...
  }, [today, id.user_id, props.user?.id]);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "attendance",
      sorter: (a, b) =>
        a.attendance?.attendance_time.length -
        b.attendance?.attendance_time.length,
      sortOrder: sortedInfo.columnKey === "attendance_time" && sortedInfo.order,
      ellipsis: false,
      width: "150px",
      render: (attendance, record, index) => {
        return <Text>{record.attendance?.attendance_time}</Text>;
      },
    },
    {
      title: "الاسم",
      ...
      render: (user_name, record) => (
        <div>
          <Avatar src={Env.HOST_SERVER_STORAGE + record.avatar}></Avatar>
  ...
  return (
    <Layout>
          ...
          <div className="generalData">
            <div className="attPer">
              <span>
                <Progress
                  type="circle"
                  percent={Math.round(attAvg)}
                  width={window.innerWidth <= 760 ? 50 : 70}
              ...
              >
                <div style={{ marginBottom: "5px" }}>
                  الحاضرون : <span>{data.length}</span>{" "}
                </div>
                <div>
                  الغائبون : <span>{abCount}</span>{" "}
                </div>
              </span>
            </div>
            <div className="disPer">
              <span>
                <Progress
                  type="circle"
                  percent={Math.round((timelyCount / data.length) * 100)}
                  width={window.innerWidth <= 760 ? 50 : 70}
                  style={{ marginLeft: "5px", display: "inline-block" }}
                />
              </span>
              ...
              >
                <div style={{ marginBottom: "5px" }}>
                  المنضبطون : <span>{timelyCount} </span>
                </div>
                <div>
                  المتأخرون : <span>{lateCount}</span>
                </div>
              </span>
            </div>
          </div>
          {type && props.user.role_id == 1 ? (
            <div className="generalOper">
              <div
                className="date"
                style={{ marginBottom: "10px", marginLeft: "5px" }}
              >
                <span>اختر يومًا : </span>{" "}
                <DatePicker
                  needConfirm={false}
              ...
              <Space style={{ display: "inline-block" }}>
                <Dropdown.Button
                  menu={menuProps}
                  trigger={["click"]}
                  style={{ backgroundColor: "#0972B6" }}
                  onClick={function () {
                    printReport("att-report");
                  }}
                  type="primary"
                >
                  <PrinterOutlined /> طباعة الحاضرين
                </Dropdown.Button>
              </Space>
            </div>
          ) : (
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                ليوم {days[new Date(today).getDay()]} الموافق {today}
              </h2>
            </div>
            <div
              style={{
                ...
                {data.map((item) => (
                  <tr
                    style={{
                      height: " 30px",
                      backgroundColor:
                        data.indexOf(item) % 2 != 0 ? "#e6e6e6" : "#fff",
                    }}
                  >
                    <td>{rank++}</td>
                    <td>{item.user_id}</td>
                ...
                {abData?.map((item) => (
                  <tr
                    style={{
                      height: " 30px",
                      backgroundColor:
                        abData.indexOf(item) % 2 != 0 ? "#e6e6e6" : "#fff",
                    }}
                  >
                    <td>{index++}</td>
                    <td>{item.user_id}</td>
              ...
              <tbody>
                {lateData.map((item) => (
                  ...
                  >
                    ...
                    <td>{item.attendance?.leave_time}</td>
                    <td>
                      {Math.floor(item.attendance?.startLateTime * -1)} دقيقة
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Modal
        ...
        onCancel={() => setShowTrophyModal(false)}
        footer={null}
        centered
        width={380}
        className="trophy-modal-custom"
        closable={false}
        maskStyle={{
      ...
      >
        <div className="trophy-modal-container">
          <div className="trophy-modal-content">
            <div className="trophy-modal-image-container">
              <img
                src={getTrophyImage(userRank)}
                alt="تكريم"
                className="trophy-modal-image"
              />
              <div className="trophy-modal-glow"></div>
            </div>

            <div className="trophy-modal-text">
              <h2 className="trophy-modal-title">
                أحسنت{" "}
                {data.find((item) => item.user_id === id.user_id)?.user_name}!
              </h2>
              <p className="trophy-modal-subtitle">
                أنت {userRank === 1 ? "أول" : userRank === 2 ? "ثاني" : "ثالث"}{" "}
                الحاضرين اليوم
              </p>
            </div>

            <Button
              className="trophy-modal-button"
