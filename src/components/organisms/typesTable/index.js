...
export default function TypesTable(props) {
  console.log("TypesTable props:", props);
  ...
  useEffect(() => {
    console.log("[useEffect] Fired. category:", category, "update:", update);
    setLoad(true);
    FirebaseServices.getTypesTasks()
      .then((data) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (period, record, index) => {
        if (record.days > 0) return parseInt(record.days) + 1;
        else return period;
      },
    },
    {
