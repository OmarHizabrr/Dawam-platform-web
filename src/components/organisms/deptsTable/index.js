...
export default function deptsTable(props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      ellipsis: false,
      width: "150px",
      render: (name, record, _) => <Text>{name}</Text>,
    },
    {
      title: "تقييم الإدارة",
      dataIndex: "att_percent",
      ...
      render: (att_percent, record, index) => {
        var star = starList?.filter(function (e) {
          return e.category_id == record.id;
        })[0]?.star;
        return (
          <>
            <Rate
    ...
    },
    {
      ...
      ellipsis: true,
      render: (tot_users, record) =>
        parseInt(record.tot_users ?? 0) + parseInt(record.tof_users ?? 0),
    },
    {
      title: "المعفيون",
      dataIndex: "tof_users",
